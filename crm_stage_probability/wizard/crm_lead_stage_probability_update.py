...
class CrmLeadStageProbabilityUpdate(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if (
            'crm_stage_update_ids' in fields_list and
            'active_ids' in self.env.context
        ):
            active_ids = self.env.context.get('active_ids')
            stages = self.env['crm.stage'].browse(active_ids)
            stages_missing_on_change = stages.filtered(
                lambda s: not s.on_change
            )
...
class CrmLeadStageProbabilityUpdateStage(models.TransientModel):
    ...
    stage_id = fields.Many2one("crm.stage", domain=[('on_change', '=', True)], readonly=True,)
    lead_count = fields.Integer("No of leads", compute='_compute_lead_count', readonly=True,)
