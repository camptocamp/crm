...
class TestCrmLeadProbability(SavepointCase):
    ...
    def test_update_probability(self):
        ...
        self.opportunity_1.write({
            'stage_id': self.stage_new.id
        })
        self.assertEqual(
            self.opportunity_1.probability, self.stage_new.probability
        )
        self.assertFalse(self.opportunity_1.is_automated_probability)
        ...
        self.assertEqual(
            ...
            self.opportunity_1.stage_id.probability
        )
        self.assertTrue(self.opportunity_1.is_stage_probability)
    ...
    def test_mass_update(self):
        ...
        wiz = self.env['crm.lead.stage.probability.update'].with_context(
            active_ids=all_stages.ids
        ).create({})
        wiz.execute()
