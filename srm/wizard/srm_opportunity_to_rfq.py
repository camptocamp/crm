...
class Opportunity2Rfq(models.TransientModel):
    ...
    def _get_selection_action(self):
        return [
            ("create", self.env._("Create a new vendor")),
            ("exist", self.env._("Link to an existing vendor")),
            ("nothing", self.env._("Do not link to a vendor")),
        ]
