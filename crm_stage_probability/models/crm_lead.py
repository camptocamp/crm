...
class CrmLead(models.Model):
    ...
    is_stage_probability = fields.Boolean(
        compute='_compute_is_stage_probability',
        readonly=True,
    )
    stage_probability = fields.Float(
    ...
    def _compute_is_stage_probability(self):
        for lead in self:
            lead.is_stage_probability = tools.float_compare(
                lead.probability, lead.stage_probability, 2
            ) == 0
    ...
    def write(self, vals):
        ...
        if (
            self.env.context.get('_auto_update_probability') and
            'probability' in vals and 'stage_id' not in vals
        ):
